...
![Tests](https://img.shields.io/badge/tests-422%20passing-2ea44f)
...
Requirements: Python 3.10+ and, for the Word report only, two packages
(python-docx, plus matplotlib for the quarterly-trend charts), pinned in
`requirements.txt`:
...
Three offline suites, no network, run in about a second - the scanner and builder suites are 422 tests total, and a report-charts suite adds 8 more:

```
cd .claude/skills/review-site/tools
python -m unittest test_review_tools        # 382 tests: parsers, graders, tool contract, pipeline, capture
cd ..
python -m unittest test_exec_report         # 40 tests: the docx builder (needs python-docx)
...
.claude/
  ...
  skills/review-site/
    ...
    report_charts.py                       Quarterly-trend chart renderer (matplotlib)
    test_exec_report.py                    Builder test suite
    test_report_charts.py                  Trend-chart renderer suite (run in CI)
    tools/                                 The evidence engine (pure standard library)
      ...
      test_review_tools.py                 Offline scanner suite (382 tests)
